...
if ($string!='' && $date_to == '' && $date_from == '') 
{
	...
	$getObjects=mysql_query("SELECT * FROM lijekarne WHERE (grad LIKE '%$string%') AND checked = ".$c.";");
...
else if($string!='' && $date_to != '' && $date_from != '')
{
	...
	$getObjects=mysql_query("SELECT * FROM lijekarne WHERE (grad LIKE '%$string%') AND timestamp <= '".$date_to."' AND timestamp >= '".$date_from."' AND checked = ".$c.";");
	while ($getObjects2 = mysql_fetch_assoc($getObjects))
	{
