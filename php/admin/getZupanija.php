...
$string = strtoupper(mysql_real_escape_string(trim($_POST['string'])));
...
{
    echo $string;
	$getObjects = mysql_query("SELECT * FROM lijekarne WHERE zupanija = '{$string}' AND checked = '{$c}';");
    print_r($getObjects);
...
else if($string!='' && $date_to != '' && $date_from != '')
{
	...
	$getObjects=mysql_query("SELECT * FROM lijekarne WHERE (zupanija LIKE '%$string%') AND timestamp <= '".$date_to."' AND timestamp >= '".$date_from."' AND checked = ".$c.";");
	while ($getObjects2 = mysql_fetch_assoc($getObjects))
	{
